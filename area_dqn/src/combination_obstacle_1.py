...
class Combination():
    ...
    def setPoint(self, x, y, m):
        ...
        if (x <= 8.0 and x > 0) and (y < -3.0 and y >= -5.0):
            ...
            y = y
        #A'
        if (x <= 0.0 and x > -7.0) and (y < -3.0 and y >= -5.0):
            x = x - nor
            y = y
        #B'
        if (x <= -6.0 and x > -7.0) and (y < -1.5 and y >= -3.0):
            x = x
            y = y + nor
        #B
        if (x <= -7.0 and x >= -9.0) and (y < -1.5 and y >= -5.0):
            x = x 
            y = y + nor
        #C
        if (x < -6.0 and x >= -9.0) and (y <= 0.0 and y >= -1.5):
            x = x + nor
            y = y
        #D
        if (x < -3.0 and x >= -6.0) and (y <= 0.0 and y >= -2.0):
            x = x + nor
            y = y
        #E
        if (x < 0.0 and x >= -3.0) and (y < 0.0 and y >= -3.0):
            x = x
            y = y + nor
        #F
        if (x < 1.0 and x >= -4.0) and (y < 3.0 and y >= 0.0):
            x = x
            y = y + nor
        #G
        if (x <= 0.0 and x >= -4.0) and (y <= 5.0 and y >= 3.0):
            x = x + nor
            y = y
        #H
        if (x <= 6.0 and x >= 0.0) and (y <= 5.0 and y > 0.0):
            if m == "A"  and (x <= 2.0 and x >= 1.5):
                ...
                y = y
        #I
        if (x <= 6.0 and x >= 0.0) and (y <= 0.5 and y > -1.5):
            x = x
            y = y - nor
        #J
        if (x < 8.0 and x >= 0.0) and (y <= -1.5 and y > -3.0):
            x = x + nor
            y = y
        #K
        if x >= 8.0 and (y <= 0.0 and y > -3.5):
            x = x
            y = y - nor
        #L
        if x > 8.0 and y <= -3.5:
        ...
        while not rospy.is_shutdown():
            ...
            for i in range(len(model.name)):
                ...
                if model.name[i] == 'obstacle_1':
                    obstacle.model_name = 'obstacle_1'
                ...
                if model.name[i] == 'obstacle_2':
                    obstacle.model_name = 'obstacle_2'
                    obstacle.pose = model.pose[i]
                    index_x = random.randrange(0, 5)
                    index_y = random.randrange(0, 5)
                    obstacle.pose.position.x, obstacle.pose.position.y = self.setPoint(obstacle.pose.position.x, obstacle.pose.position.y, m)
                ...
                if model.name[i] == 'obstacle_3':
                    obstacle.model_name = 'obstacle_3'
                    obstacle.pose = model.pose[i]
                    index_x = random.randrange(0, 5)
                    index_y = random.randrange(0, 5)
                    obstacle.pose.position.x, obstacle.pose.position.y = self.setPoint(obstacle.pose.position.x, obstacle.pose.position.y, m)
                ...
                if model.name[i] == 'obstacle_4':
                    obstacle.model_name = 'obstacle_4'
                    obstacle.pose = model.pose[i]
                    index_x = random.randrange(0, 5)
                    index_y = random.randrange(0, 5)
                    obstacle.pose.position.x, obstacle.pose.position.y = self.setPoint(obstacle.pose.position.x, obstacle.pose.position.y, m)
                ...
                if model.name[i] == 'obstacle_5':
                    obstacle.model_name = 'obstacle_5'
                    obstacle.pose = model.pose[i]
                    index_x = random.randrange(0, 5)
                    index_y = random.randrange(0, 5)
                    obstacle.pose.position.x, obstacle.pose.position.y = self.setPoint(obstacle.pose.position.x, obstacle.pose.position.y, m)
